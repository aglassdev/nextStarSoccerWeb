...
// ── Types ─────────────────────────────────────────────────────────────────────
interface FormData {
  // Step 1 — Applicant (parent/guardian)
...
const EMPTY: FormData = {
  ...
  playerGrade: '', playerSchool: '', playerPosition: '', trainingHistory: '',
  householdSize: '', annualHouseholdIncome: '', employmentStatus: '',
  receivesAssistance: '', assistanceDetails: '',
  requestedAid: '', personalStatement: '', referralSource: '', coachReference: '',
};
...
export default function ScholarshipApplication({ onClose }: Props) {
  ...
  const handleFileSelect = async (key: string, file: File) => {
    ...
    try {
      ...
      setFiles(prev => ({
        ...prev,
        [key]: { file, label: key, uploading: false, error: e.message || 'Upload failed' },
      }));
    }
  };
  ...
  const validate = (): string => {
    ...
    if (step === 2) {
      if (!form.householdSize.trim())          return 'Household size is required';
      if (!form.annualHouseholdIncome.trim())  return 'Annual household income is required';
    }
    if (step === 3) {
      if (!form.requestedAid)               return 'Please select the aid amount requested';
      ...
      for (const doc of requiredDocs) {
        ...
        if (!f || f.uploading || f.error) return `Please attach: ${doc.label}`;
      }
    }
    return '';
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await databases.createDocument(
        databaseId,
        collections.scholarshipApplications,
        ID.unique(),
        {
          ...form,
      ...
      <ModalShell onClose={onClose}>
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-gray-400 text-sm max-w-xs leading-relaxed mb-8">
            Thank you, {form.applicantFirstName}. Our scholarship committee will review your application and contact you at <span className="text-white">{form.applicantEmail}</span> within 2–3 weeks.
          </p>
          <button
            onClick={onClose}
    ...
    <ModalShell onClose={onClose}>
      ...
      {step === 0 && (
        <div className="space-y-4">
          <p className="text-gray-500 text-xs mb-2">Information about the parent or guardian submitting this application.</p>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <p className="text-gray-500 text-xs mb-2">Information about the player applying for the scholarship.</p>
          <div className="grid grid-cols-2 gap-4">
            <Field label="First Name" required>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Primary Position">
              <Select value={form.playerPosition} onChange={e => set('playerPosition', e.target.value)}>
                <option value="">Select position</option>
                {['Goalkeeper','Defender','Midfielder','Forward','Multiple'].map(p => (
      ...
      {step === 2 && (
        <div className="space-y-4">
          <p className="text-gray-500 text-xs mb-2">All financial information is kept strictly confidential and used only for scholarship review.</p>
          <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Employment Status">
            <Select value={form.employmentStatus} onChange={e => set('employmentStatus', e.target.value)}>
              <option value="">Select</option>
              {['Employed full-time','Employed part-time','Self-employed','Unemployed','Retired','Disabled / Unable to work'].map(s => (
                <option key={s} value={s}>{s}</option>
              ))}
            </Select>
          </Field>
          <Field label="Does your family currently receive government assistance?">
          ...
          {form.receivesAssistance === 'yes' && (
            <Field label="Please specify (e.g. SNAP, Medicaid, WIC, Section 8…)">
              <Textarea
                ...
                placeholder="Optional — list any programs"
              />
            </Field>
          )}
        </div>
      ...
      {step === 3 && (
        <div className="space-y-5">
          <Field label="Aid Requested" required>
            <div className="flex gap-3">
              {[['full','Full Scholarship'],['partial','Partial Scholarship']].map(([val, label]) => (
                <button
                  key={val}
                  type="button"
                  onClick={() => set('requestedAid', val)}
                  className={`flex-1 py-2.5 rounded-lg border text-sm font-medium transition-colors ${
                    form.requestedAid === val
          ...
          <Field label="How did you hear about NSS?">
            <Input value={form.referralSource} onChange={e => set('referralSource', e.target.value)} placeholder="e.g. Coach, social media, friend…" />
          </Field>

          <Field label="Coach or Reference Name (if applicable)">
          ...
          {[
            ...
            },
            {
              ...
              rows: [
                ...
                ['Training History', form.trainingHistory],
              ],
            },
            {
              title: 'Financial',
              rows: [
                ...
                ['Receives Assistance', form.receivesAssistance],
            ...
            },
            {
              title: 'Application',
              rows: [
                ['Aid Requested',   form.requestedAid],
                ['Referral Source', form.referralSource],
                ['Coach Reference', form.coachReference],
            ...
            {DOC_SLOTS.map(slot => {
              ...
              return (
                <div key={slot.key} className="flex items-center justify-between py-1.5 border-b border-[#1a1a1a] last:border-0">
                  <span className="text-gray-500 text-xs w-48 flex-shrink-0">{slot.label}</span>
                  {f?.fileId ? (
...
function ModalShell({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto py-8 px-4">
      ...
      <div className="relative w-full max-w-xl bg-[#0a0a0a] border border-[#1e1e1e] rounded-2xl shadow-2xl z-10 p-6 md:p-8 my-auto">
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1.5 text-gray-600 hover:text-white transition-colors rounded-lg hover:bg-white/5"
        >
